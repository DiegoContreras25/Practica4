...
import trabajadorSchema from "../db/trabajador.ts";
...
export const deleteTrabajador = async (
  ...
  try {
    ...
    const id = req.params.id;
    const subject = await trabajadorSchema.findByIdAndDelete(id).exec();
    ...
    res.status(200).send("Trabajador deleted");
  } catch (error) {
